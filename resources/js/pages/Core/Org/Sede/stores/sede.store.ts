...
export const useSedeStore = defineStore('orgSedes', {
  state: () => ({
  ...
  actions: {
    ...
    async list() {
      ...
      inflight = (async () => {
        try {
          const { data } = await axios.get(route('org.sede.datatable'));
    ...
    async save(form: { id?: number; nombre: string; departamento?: number }) {
      try {
        if (form.id) {
          ...
          await axios.put(route('org.sede.update', form.id), { nombre: form.nombre, departamento_id: form.departamento });
        } else {
          // Crea nuevo departamento
          await axios.post(route('org.sede.store'), { nombre: form.nombre, departamento_id: form.departamento });
        }
