...
async function eliminar() {
  ...
  confirm.require({
    ...
    accept: async () => {
      deleting.value = true;
      try {
        await axios.delete(route('org.sede.destroy', form.value.id));
        toast.add({
          severity: 'success',
          summary: 'Eliminado',
...
<template>
  ...
  <Dialog v-model:visible="visible" :header="form.id ? 'Editar Sede' : 'Nuevo Sede'" modal>
    <div class="grid grid-cols-2 gap-3">
     
      <div class="flex flex-col gap-2">
        <label for="sede">Sede</label>
        <InputText id="sede" v-model="form.nombre" aria-describedby="sede-help" />
      </div>
    </div>
    <template #footer>
